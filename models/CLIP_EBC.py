...
from .clip.image_encoder import VisionTransformer
...
def _vit(name: str, features_only: bool = False, input_size: Optional[Union[int, Tuple[int, int]]] = None) -> VisionTransformer:
...
class CLIP_EBC(nn.Module):
    def __init__(
        ...
        self.image_encoder = _vit("vit_b_16", features_only=True, input_size=input_size)
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        ...
        if self.reduction != self.encoder_reduction:
            print("Scale Factor: ", self.encoder_reduction/self.reduction)
